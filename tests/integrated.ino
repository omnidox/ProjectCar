...
//******Please input the code of your approaches/algorithms in the below area*****************//
  ...
  // Calculate the area, primarily used for steering
  float front_left_area = distance2 * distance3; // Use the front-left-most sensors (sensors 2, 3)
  float front_right_area = distance4 * distance5; // Use the front-right-most sensors (sensors 4, 5)

  // Retrieve difference to determine severity of steering angle
  float difference = abs(front_left_area - front_right_area);
  ...
  // Stop
  if (distance3 <= STOP_DISTANCE || distance4 <= STOP_DISTANCE) {
    velocity = STATIONARY;
  }
  // Turn right
  else if (front_left_area < front_right_area) {
    steering += calculate_steering_angle(difference);
  }
  // Turn left
  else {
    steering -= calculate_steering_angle(difference);
  ...
  steering = constrain(steering, MIN_STEER, MAX_STEER);

  setVehicle(steering, velocity);
}

float calculate_steering_angle(float difference) {
  return (difference / 100) * STEER_SENSITIVITY;
}
